...
do_secret_action :: proc(action: SecretAction) {
	...
	_, out, _, _ := os.process_exec({command = pass_command}, context.allocator)

	out_s := string(out)
	if strings.starts_with(out_s, "otpauth://") {
		pass_command_otp := []string{"sh", "-c", fmt.tprintf("pass otp %s", action.name)}
		_, out_otp, _, _ := os.process_exec({command = pass_command_otp}, context.allocator)
		_, _ = os.process_start({command = {"wl-copy", strings.trim(string(out_otp), "\n")}})
	} else {
		_, _ = os.process_start({command = {"wl-copy", strings.trim(string(out_s), "\n")}})
	}
...
get_secret_actions :: proc() -> []Action {
	...
	secrets_path := fmt.tprintf("%s/.password-store", os.get_env("HOME", context.allocator))
	home, _ := os.open(secrets_path)
	fis, _ := os.read_dir(home, -1, context.allocator)
